...
async function api(request,env){
 ...
 if(path==="/api/reviews"&&request.method==="POST"){const user=await userFrom(request,env);if(!user)return json({error:"Authentication required"},401);const b=await request.json().catch(()=>({})),rating=Number(b.rating);if(!b.product_id||rating<1||rating>5)return json({error:"Product and rating 1-5 required"},400);await env.DB.prepare("INSERT INTO reviews(id,product_id,user_id,rating,comment) VALUES(?,?,?,?,?)").bind(crypto.randomUUID(),Number(b.product_id),user.id,rating,String(b.comment||"")).run();return json({ok:true},201)}
